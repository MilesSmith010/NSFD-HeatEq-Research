%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   NSFD Schemes ID #28 using Forward Difference Method for Heat Equation
%   Explicit Equation:   
...
%
%    (CASE IV)
%    ID#28 Unity Approximation:
%          1 = (U(j,i) + U(j,i+1) + U(j,i-1) + U(j,i))/(2U(j,i)
%
%          U(j+1,k) = 1/2(1-2*R)U(j,i) + (1-R)(U(j,i+1) + U(j,i-1))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j=1:K
    for i=2:M
          u(j+1,i) = 1/2*(1-2*R)*u(j,i) + (1-R)*(u(j,i+1) + u(j,i-1));
    end
end
